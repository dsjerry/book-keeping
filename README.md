...
- [ ] **收编 API 对接分支**：`feat/api-integration` 真机验证通过（登录 → 同步 → 冲突解决 → 图片云备份 → AI 代理全流程）后合并回 `dev`，随后删除该分支
- [ ] **不用分支区分"有后端/无后端"**：app 是本地优先架构，同一个包通过「启用同步」开关即可在两种模式下运行（记账纯本地、同步失败自动退化离线、图片/AI 均有降级路径）；分支只区分代码成熟度，版本发布用 tag

### 🧭 双模式体验

- [ ] **设置页显示当前模式**：「纯本地模式 / 已连接服务端」（依据 `useOnline` + `currentUser.serverId`），让用户明确知道自己处在哪种形态
- [ ] **已知边界（记录，不需动作）**：先启用同步后切回纯本地时，图片依赖服务端地址——本地裁剪文件仍在则无感；清缓存且无服务端则历史图片不可见。当前"失败回退本地路径"已是最合理处理

### 📡 服务端能力接入（接口已就绪，待接）

- [ ] **图表接 `/v1/analysis/summary`**：分析页图表在联网时改用服务端聚合数据，离线回退本地计算
- [ ] **分类定义上云**：自定义 tag 的增删改同步到 `/v1/category`，换设备后图标/颜色可恢复（记账记录里的 tags 别名已随同步落库）

### 🧹 技术债

- [ ] **`App.test.tsx`**：整树渲染测试需补齐导航/原生模块 mock 链（其余测试套件已恢复）
- [ ] **本地用户密码明文存储**：`userStore` 应只存哈希，或引导用户依赖服务端账号
- [ ] **清理死代码**：`useFetch`、`api/test.ts`；README/文档中的版本号与同步流程描述同步更新
