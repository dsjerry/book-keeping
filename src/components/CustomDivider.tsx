...
const CustomDivider: React.FC<Props> = ({ style, text = '', textPosi = 'center' }) => {
  // 获取当前主题
  const theme = useTheme()
  const LeftDivider = () => (textPosi === 'right' || textPosi === 'center' ? <Divider style={{ flex: 1 }} /> : <></>)
  const RightDivider = () => (textPosi === 'left' || textPosi === 'center' ? <Divider style={{ flex: 1 }} /> : <></>)
